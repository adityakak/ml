...
def g0_search():
    direction = [x[:] for x in [[("", float('-inf'), float('inf'))] * width] * height]
    locations = []
        ...
        for j in range(width):
            if game_map[i][j]['Reward'] > 0:
                ...
                for loc in locations:
                    distance_map = loc[2]
                    answer = ""
                    if loc[0] >= direction[i][j][1] and twoToOne(i, j) in distance_map and distance_map[
                        twoToOne(i, j)] <= direction[i][j][2]:
                        ...
                        if distance_map[twoToOne(i, j)] == direction[i][j][2]:
                            direction[i][j] = (answer + direction[i][j][0], loc[0], distance_map[twoToOne(i, j)])
                        else:
                            direction[i][j] = (answer, loc[0], distance_map[twoToOne(i, j)])
    for i in range(height):
        for j in range(width):
...
def main():
    ...
    for i in range(start_index, len(args)):
        if args[i] == 'G0' or args[i] == 'g0':
            search = True
        elif args[i] == 'G1' or args[i] == 'g1':
            search = False
        else:
